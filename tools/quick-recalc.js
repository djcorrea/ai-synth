/**
 * Funk Mandela Quick Recalc - Versão simplificada para reprocessar o banco de referência
 */
...
// Usar o ref-calibrator existente que já funciona
async function runRecalibration() {
  console.log('🎵 Reprocessando Funk Mandela - Quick Recalc');
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      const samplesPath = path.resolve('refs/funk_mandela/samples');
      const proc = spawn('node', ['tools/ref-calibrator.js', 'funk_mandela', samplesPath, '--write'], { 
    ...
    if (fs.existsSync(jsonPath)) {
      ...
      console.log(`Faixas: ${data.funk_mandela?.num_tracks || 'N/A'}`);
      console.log(`Data: ${data.funk_mandela?.generated_at || 'N/A'}`);
      
      if (data.funk_mandela?.legacy_compatibility) {
        const compat = data.funk_mandela.legacy_compatibility;
        console.log('\n🎯 === NOVOS TARGETS ===');
      ...
      console.log('Sistema atualizado com novo banco de referência do Funk Mandela');
...
// Executar
if (import.meta.url === `file://${process.argv[1]}` || process.argv[1].endsWith('quick-recalc.js')) {
  runRecalibration();
}
