...
// - Usa tolerâncias da referência sempre que disponível; senão aplica fallbacks

// 🔬 Sistema de logging com throttling para diagnóstico
const diagLog = window.diagLog || function(stage, msg, ctx) {
...
function _computeEqualWeightV3(analysisData) {
  diagLog('SCORING', 'Iniciando cálculo com pesos iguais', { method: 'equal_weight_v3' });
  ...
  diagLog('SCORING', 'Valores das métricas:', metricValues);
  ...
  for (const [key, value] of Object.entries(metricValues)) {
    if (targets[key] !== undefined && tolerances[key] !== undefined && Number.isFinite(value)) {
      ...
      diagLog('SCORING_METRICS', `${key}: ${value} -> ${metricScore.toFixed(1)}% (dev: ${deviationRatio.toFixed(2)}x)`, { 
        ...
        metricScore: parseFloat(metricScore.toFixed(1)),
        deviation: parseFloat(deviationRatio.toFixed(2))
      });
    }
  }
  ...
  diagLog('SCORING', 'Resultado final:', result);
