...
function _computeEqualWeightV3(analysisData) {
  ...
  // Garantir que o score é válido
  const validScore = Number.isFinite(scoreDecimal) ? scoreDecimal : 50;
...
function _computeMixScoreInternal(technicalData = {}, reference = null, force = { AUDIT_MODE:false, SCORING_V2:false, AUTO_V2:true }) {
  ...
  try { __caiarLog('NEW_EQUAL_WEIGHT_SCORING', 'Score calculado com pesos iguais', { 
    ...
    try {
      ...
      if (equalWeightResult && Number.isFinite(equalWeightResult.score)) {
        ...
        console.error('[EQUAL_WEIGHT_V3] ❌ Resultado inválido:', equalWeightResult);
        throw new Error('equalWeightResult inválido');
      }
