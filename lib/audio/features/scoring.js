...
// 🎯 TOLERÂNCIAS AJUSTADAS PARA SEREM MAIS REALÍSTICAS
// Evita que microdiferenças irrelevantes derrubem o score
const DEFAULT_TARGETS = {
  ...
  lufsIntegrated: { target: -14, tol: 1.5 },  // ↑ Era 1.0 - Mais tolerante para LUFS
  lra: { target: 7, tol: 4 },                 // ↑ Era 3.0 - Dinâmica mais flexível
  dr: { target: 10, tol: 4 },                 // ↑ Era 3.0 - Dynamic Range mais flexível
  truePeakDbtp: { target: -1, tol: 1.5, invert: true }  // ↑ Era 1.0 - Peak mais tolerante
};

// Mantido para compatibilidade (não mais usado na fórmula final principal, mas
// 🎯 PESOS REBALANCEADOS PARA SCORE MAIS REALÍSTICO
// Ajustados para refletir melhor a experiência do usuário final
const CATEGORY_WEIGHTS = {
  loudness: 20,   // ↑ Era 15 - LUFS é fundamental para experiência
  dynamics: 20,   // = Mantém - Dinâmica é crucial
  peak: 15,       // ↑ Era 10 - Clipping é muito prejudicial  
  stereo: 10,     // = Mantém - Aspecto complementar
  tonal: 20,      // ↓ Era 25 - Reduz dominância excessiva
  spectral: 10,   // = Mantém - Análise técnica complementar
  technical: 5    // ↓ Era 10 - Detalhes técnicos menos críticos
};
...
function _computeMixScoreInternal(technicalData = {}, reference = null, force = { AUDIT_MODE:false, SCORING_V2:false, AUTO_V2:true }) {
  ...
  // ================== MÉTODO PRINCIPAL COLOR_RATIO_V2 ==================
  // Verificar se a flag SCORING_COLOR_RATIO_V2 permite o uso deste método
  const colorRatioEnabled = (() => {
    // Ler flag da feature-flags ou window
    if (typeof window !== 'undefined' && window.SCORING_COLOR_RATIO_V2 !== undefined) {
      return window.SCORING_COLOR_RATIO_V2;
    }
    // Por padrão, usar color_ratio_v2 (como especificado nas flags)
    return true;
  })();
  
  if (colorRatioEnabled) {
    try {
  // CORREÇÃO FINAL: usar perMetric que contém as classificações calculadas
  // Garantir que temos métricas processadas primeiro
  console.log('[COLOR_RATIO_V2_DEBUG] perMetric length:', perMetric.length);
  console.log('[COLOR_RATIO_V2_DEBUG] perMetric sample:', perMetric.slice(0, 3).map(m => ({ key: m.key, status: m.status, severity: m.severity })));
  
  if (perMetric.length === 0) {
    console.warn('[COLOR_RATIO_V2] Nenhuma métrica em perMetric - algo está errado no processamento');
    throw new Error('Nenhuma métrica processada');
  }
  
  // Usar todas as métricas processadas (como era originalmente)
  const visibleFinal = perMetric;
  const denomKeys = visibleFinal.map(m=>m.key);
  const total = visibleFinal.length;
  const green = visibleFinal.filter(m=>m.status==='OK').length;
  const yellow = visibleFinal.filter(m=>m.status!=='OK' && m.severity==='leve').length;
  ...
  console.log('[COLOR_RATIO_V2_INTERNAL] Contagens:', { total, green, yellow, red });
    ...
    // 🎯 FÓRMULA MELHORADA: Verde=1.0, Amarelo=0.7, Vermelho=0.3 (mais realístico que 1.0/0.5/0.0)
    const raw = total > 0 ? ((green * wGreen + yellow * wYellow + red * wRed) / total) * 100 : 0;
    result.scorePct = Math.round(raw);
    result.score_simple_binary = total>0 ? Math.round((green/total)*100) : 0;
    result.method = 'color_ratio_v2';
    result.scoringMethod = 'color_ratio_v2';
    result.colorCounts = { green, yellow, red, total };
    result.weights = { green: wGreen, yellow: wYellow, red: wRed };
    result.denominator_info = denomKeys;
    
    // INSTRUMENTAÇÃO PARA DIAGNÓSTICO
    result.yellowKeys = visibleFinal.filter(m=>m.status!=='OK' && m.severity==='leve').map(m=>m.key);
    result.greenKeys = visibleFinal.filter(m=>m.status==='OK').map(m=>m.key);
    result.redKeys = visibleFinal.filter(m=>m.status!=='OK' && m.severity!=='leve').map(m=>m.key);
    
    result.audit = { rawExact: raw, greenPortion: total?green/total:0, yellowPortion: total?yellow/total:0 };
    result.colorDebug = colorDebug;
    result.previousAdvancedScorePct = result.advancedScorePct;
    result.classification = classify(result.scorePct);
    try { __caiarLog('SCORING_COLOR_COUNTS','Contagem cores (color_ratio_v2)', { green, yellow, red, total, weights: result.weights, rawExact: raw, scorePct: result.scorePct }); } catch {}
    try { __caiarLog('SCORING_DENOMINATOR_INFO','Chaves usadas', { keys: denomKeys }); } catch {}
    try { __caiarLog('SCORING_METHOD_SELECTED','Método principal', { method: 'color_ratio_v2', scorePct: result.scorePct, advancedScorePct: result.advancedScorePct }); } catch {}
    try { __caiarLog('SCORING_COLOR_DEBUG','Detalhe por métrica', { items: colorDebug }); } catch {}
  } catch (eColor) {
    ...
    result.classification = classify(result.scorePct);
    try { __caiarLog('SCORING_COLOR_ERROR','Falha color_ratio_v2 -> fallback advanced', { error: eColor?.message, fallback_used: 'advanced' }); } catch {}
  }
  } else {
    // Flag SCORING_COLOR_RATIO_V2 desabilitada, usar apenas advanced
    result.scorePct = result.advancedScorePct;
    result.method = 'advanced_only';
    result.scoringMethod = 'advanced_only';
    result.classification = classify(result.scorePct);
    try { __caiarLog('SCORING_COLOR_DISABLED','Color ratio v2 desabilitado, usando advanced', { scorePct: result.scorePct }); } catch {}
  }
...
if (typeof window !== 'undefined') { 
  window.__MIX_SCORING_VERSION__ = '1.0.1-color-ratio-v2'; 
  // Global namespace para browser
  window.ScoringV1 = {
    computeMixScore: computeMixScore,
    computeMixScoreBoth: computeMixScoreBoth
  };
  console.log('🎯 [SCORING_V1] Módulo V1 carregado globalmente!');
}
