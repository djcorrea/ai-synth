// 🧮 MIX SCORING ENGINE V3 - EQUAL WEIGHT SYSTEM
// Calcula porcentagem de conformidade usando pesos iguais para todas as métricas
// Design principles:
...
// - Peso igual para todas as métricas (mais justo que sistema de categorias)
...
// Mantido para compatibilidade com fallbacks avançados
// 🎯 PESOS LEGADOS - USADOS APENAS EM FALLBACK AVANÇADO
...
function _computeEqualWeightV3(analysisData) {
  ...
  const metricValues = {
    ...
    // 🔧 CORREÇÃO CRÍTICA: Substituir centroide único por bandas individuais
    // centroid: metrics.centroid || metrics.spectral_centroid || 2500, // REMOVIDO
    
    // Adicionar bandas individuais de frequência
  ...
  const targets = {
    ...
    stereoWidth: 0.6,
    balanceLR: 0,
    // 🔧 CORREÇÃO: Targets das bandas individuais
  ...
  const tolerances = {
    ...
    stereoWidth: 0.3,
    balanceLR: 0.2,
    // 🔧 CORREÇÃO: Tolerâncias das bandas individuais
...
function _computeMixScoreInternal(technicalData = {}, reference = null, force = { AUDIT_MODE:false, SCORING_V2:false, AUTO_V2:true }) {
  ...
  const result = {
    ...
    formulaNote: 'advancedScorePct = 100 * (1 - max(P_sum, P_crit)); método principal de exibição = equal_weight_v3.'
  };
  
  // ================== SISTEMA EQUAL_WEIGHT_V3 ATIVO ==================
  // Sistema de pesos iguais é o padrão - código legacy removido
  console.log('[EQUAL_WEIGHT_V3] Usando sistema de pesos iguais como padrão');
...
if (typeof window !== 'undefined') { 
  window.__MIX_SCORING_VERSION__ = '3.0.0-equal-weight-v3-FINAL'; 
  console.log('🎯 SISTEMA FINAL CARREGADO - Versão:', window.__MIX_SCORING_VERSION__);
}
