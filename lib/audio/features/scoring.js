...
// - Usa tolerâncias da referência sempre que disponível; senão aplica fa    console.log('[COLOR_RATIO_V2_INTERNAL] Contagens:', { total, green, yellow, red });
...
function _computeEqualWeightV3(analysisData) {
  ...
  if (scoreDecimal >= 85) classification = 'Referência Mundial';
  else if (scoreDecimal >= 70) classification = 'Avançado';
  else if (scoreDecimal >= 55) classification = 'Intermediário';
  ...
  const result = {
    ...
    details: {
      ...
      equalWeight: parseFloat((100 / metricCount).toFixed(2)),
      metricDetails: details
    }
  };
  
  console.log('[EQUAL_WEIGHT_V3] ✅ Resultado final:', result);
...
function _computeMixScoreInternal(technicalData = {}, reference = null, force = { AUDIT_MODE:false, SCORING_V2:false, AUTO_V2:true }) {
  ...
  // 🎯 NOVO SISTEMA DE PENALIDADES BALANCEADAS V4
  // Reformulado para penalidades mais suaves e realísticas:
  // - Clipping: máximo 15 pontos de penalidade
  // - Loudness: máximo 10 pontos de penalidade  
  // - Correlação: máximo 8 pontos de penalidade
  // - Dynamic Range: máximo 5 pontos de penalidade
  function unitPenaltyFromN(n){
    if (n <= 0) return 0;
    if (n <= 1) return 0.05 * n; // Muito suave para diferenças pequenas
    if (n <= 2) return 0.05 + 0.10 * (n - 1); // 0.05 -> 0.15 (suave)
    if (n <= 3) return 0.15 + 0.15 * (n - 2); // 0.15 -> 0.30 (moderado)
    // n > 3: 0.30 + 0.20 * ((n-3)/2) saturando em 0.50 em n=5
    return 0.30 + 0.20 * clamp01f((n - 3) / 2);
  }

  // 🎯 PESOS REFORMULADOS PARA PENALIDADES BALANCEADAS
  // Redistribuição para que cada categoria tenha impacto mais equilibrado
  const baseWeights = {
    // Clipping: peso alto mas penalidade limitada (max 15 pontos)
    clippingPct: 0.15,           // Era implícito, agora explícito
    clippingSamples: 0.15,       // Era implícito, agora explícito
    
    // Loudness: peso moderado, penalidade limitada (max 10 pontos)
    lufsIntegrated: 0.10,        // Reduzido de 0.18 para 0.10
    truePeakDbtp: 0.10,          // Reduzido de 0.14 para 0.10
    
    // Correlação/Stereo: peso moderado, penalidade limitada (max 8 pontos)
    stereoCorrelation: 0.08,     // Aumentado para dar mais peso
    stereoWidth: 0.05,           // Complementar ao stereoCorrelation
    
    // Dynamic Range: peso baixo, penalidade limitada (max 5 pontos)
    dr: 0.05,                    // Peso baixo para impacto limitado
    dr_stat: 0.05,               // Alternativo ao dr
    lra: 0.05,                   // Relacionado à dinâmica
    
    // Bandas espectrais: peso distribuído uniformemente
    band_high_mid: 0.08,         // Reduzido de 0.14 para 0.08
    band_brilho: 0.08,           // Reduzido de 0.12 para 0.08
    band_presenca: 0.08,         // Reduzido de 0.12 para 0.08
    band_low_bass: 0.06,         // Reduzido de 0.08 para 0.06
    band_upper_bass: 0.05,       // Reduzido de 0.07 para 0.05
    band_low_mid: 0.04,          // Reduzido de 0.05 para 0.04
    band_mid: 0.04,              // Reduzido de 0.05 para 0.04
    band_sub: 0.03               // Novo, menor peso
  };

  // 🎯 FUNÇÃO DE PENALIDADE ESPECÍFICA POR TIPO DE PROBLEMA
  function calculateSpecificPenalty(metricType, deviationFactor) {
    ...
    const maxPenalty = maxPenalties[category] || 0.10; // Default 10 pontos
    
    // Curva suave: penalidade cresce gradualmente até o máximo
  ...
  function getMetricCategory(metricType) {
    if (metricType.includes('clipping') || metricType.includes('clip')) return 'clipping';
    if (metricType.includes('lufs') || metricType.includes('truePeak')) return 'loudness';
    if (metricType.includes('stereo') || metricType.includes('correlation')) return 'correlation';
    if (metricType.includes('dr') || metricType.includes('lra') || metricType.includes('dynamic')) return 'dynamics';
    return 'other';
  }
  ...
  for (const m of presentMetrics) {
    ...
    // 🎯 NOVO: Usar sistema de penalidades balanceadas
    const deviationFactor = Number.isFinite(m.n) ? m.n : 0;
    const specificPenalty = calculateSpecificPenalty(m.key, deviationFactor);
    const u = specificPenalty; // Usar nova função ao invés de unitPenaltyFromN
    ...
    penalties.push({ 
      ...
      maxPenalty: u / (w || 0.01) // Penalidade máxima possível para esta métrica
    });
  }
  const P_sum = penalties.reduce((a,b)=> a + b.p, 0);
  
  // 🎯 NOVO: Sistema de penalidade crítica mais balanceado
  // Separar penalidades por categoria para aplicar caps específicos
  const penaltiesByCategory = {
    clipping: penalties.filter(p => p.category === 'clipping').reduce((a,b) => a + b.p, 0),
    loudness: penalties.filter(p => p.category === 'loudness').reduce((a,b) => a + b.p, 0),
    correlation: penalties.filter(p => p.category === 'correlation').reduce((a,b) => a + b.p, 0),
    dynamics: penalties.filter(p => p.category === 'dynamics').reduce((a,b) => a + b.p, 0),
    other: penalties.filter(p => !['clipping','loudness','correlation','dynamics'].includes(p.category)).reduce((a,b) => a + b.p, 0)
  };
  
  // Aplicar caps por categoria (em pontos percentuais)
  const cappedPenalties = {
    ...
    other: Math.min(penaltiesByCategory.other, 0.12)           // Max 12 pontos para outras
  };
  
  const P_final_capped = Object.values(cappedPenalties).reduce((a,b) => a + b, 0);
  
  // 🎯 FÓRMULA FINAL BALANCEADA: Score nunca vai abaixo de 30, penalidades suaves
  const scoreNew = Math.max(30, 100 - (P_final_capped * 100)); // Floor de 30% e penalidades em pontos diretos
  
  console.log('[SCORING_V4] 🎯 Penalidades por categoria:', {
    clipping: `${(penaltiesByCategory.clipping * 100).toFixed(1)} -> ${(cappedPenalties.clipping * 100).toFixed(1)} pontos`,
    loudness: `${(penaltiesByCategory.loudness * 100).toFixed(1)} -> ${(cappedPenalties.loudness * 100).toFixed(1)} pontos`,
    correlation: `${(penaltiesByCategory.correlation * 100).toFixed(1)} -> ${(cappedPenalties.correlation * 100).toFixed(1)} pontos`,
    dynamics: `${(penaltiesByCategory.dynamics * 100).toFixed(1)} -> ${(cappedPenalties.dynamics * 100).toFixed(1)} pontos`,
    other: `${(penaltiesByCategory.other * 100).toFixed(1)} -> ${(cappedPenalties.other * 100).toFixed(1)} pontos`,
    totalPenalty: `${(P_final_capped * 100).toFixed(1)} pontos`,
    finalScore: `${scoreNew.toFixed(1)}%`
  });
  // ================== RESULT STRUCT ==================
  const result = {
    ...
    scorePct: null,
    classification: null,
    scoreMode: SCORING_V2 ? 'v2_balanced' : 'legacy', // Indicar novo sistema
    ...
    perMetric: perMetric.map(m=>({ 
      ...
      severity: m.severity, 
      n: m.n, 
      scorePct: parseFloat((m.score*100).toFixed(1)) 
    })),
    highlights: { /* placeholders antes: best/worst removidos */ },
    penalties,
    penaltiesByCategory: {
      raw: penaltiesByCategory,
      capped: cappedPenalties,
      totalCapped: P_final_capped
    },
    penaltiesSummary: { 
      P_sum: parseFloat(P_sum.toFixed(4)), 
      P_final_capped: parseFloat(P_final_capped.toFixed(4)),
      maxPenaltyLimits: {
        clipping: 15,
        loudness: 10, 
        correlation: 8,
    ...
    formulaNote: 'advancedScorePct = max(30, 100 - penaltiesSum); penalidades limitadas por categoria; método balanceado v4.'
  };
  ...
  try { __caiarLog('NEW_EQUAL_WEIGHT_SCORING', 'Score calculado com pesos iguais', { 
    ...
    try {
      ...
      const analysisData = {
        ...
        reference: reference,
        runId: technicalData.runId || 'scoring-' + Date.now()
      };
...
export function computeMixScore(technicalData = {}, reference = null) {
  ...
  // 🚨 DEBUG: Verificar se novo sistema V4 está ativo
  console.log('[SCORING_V4] 🔥 NOVO SISTEMA DE PENALIDADES BALANCEADAS ATIVO!');
  console.log('[SCORING_V4] 📊 Limites: Clipping=15pts, Loudness=10pts, Correlation=8pts, Dynamics=5pts');
  ...
  const result = _computeMixScoreInternal(technicalData, reference, { AUDIT_MODE, SCORING_V2, AUTO_V2, overrideAuditBypass });
  
  console.log('[SCORING_ENTRY] 📊 Resultado final:', {
