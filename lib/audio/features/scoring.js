...
function _computeMixScoreInternal(technicalData = {}, reference = null, force = { AUDIT_MODE:false, SCORING_V2:false, AUTO_V2:true }) {
  ...
  // Combinação crítica: suaviza segunda maior
  const P_crit = Math.max(Ucrit_max, (Ucrit_max*0.7 + Ucrit_2nd*0.3));
  const P_final = Math.min(1, Math.max(P_sum, P_crit));
  const scoreNew = (1 - P_final) * 100;
  // ================== RESULT STRUCT ==================
  const result = {
  ...
  console.log('[COLOR_RATIO_V2_INTERNAL] Contagens:', { total, green, yellow, red });
    ...
    const wYellow = Number.isFinite(winCfg.SCORE_WEIGHT_YELLOW)? winCfg.SCORE_WEIGHT_YELLOW : 0.5;
    const wRed = Number.isFinite(winCfg.SCORE_WEIGHT_RED)? winCfg.SCORE_WEIGHT_RED : 0.0;
    
    // FÓRMULA OFICIAL E ÚNICA: mixScorePct = round(100 * ((1.0*G + 0.5*Y + 0.0*R) / T))
