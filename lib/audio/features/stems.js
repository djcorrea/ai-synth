// Stems Separation Module (heuristic fallback)
// Objetivo: disponibilizar 4 grupos (bass, drums, vocals, other) em <=90s.
// Estratégia: tenta serviço externo (window.STEMS_SERVICE_URL) -> fallback heurístico rápido (single pass) -> null.
// Apenas ativa pipeline adicional se CAIAR_ENABLED=true para não impactar fluxo padrão.
...
export async function separateStems(audioBuffer, options={ timeoutMs:90000 }) {
  ...
  try {
    caiarLog('STEMS_START','Iniciando separação');
